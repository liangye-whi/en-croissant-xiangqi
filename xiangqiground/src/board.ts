...
import { pos2key, opposite, distanceSq, allPos, computeSquareCenter } from './util.js';
...
export function userMove(state: HeadlessState, orig: cg.Key, dest: cg.Key): boolean {
  //alert(canMove(state, orig, dest));
  //if (state.movable.dests){
  //  alert(`${[...state.movable.dests.keys()]},${orig},${dest}`);
  //}
  ...
  const legalDests = state.movable.dests?.get(orig) ?? [];
  safeInfo(
    `[Xiangqiground] move rejected orig=${orig} dest=${dest}; legal=${
      legalDests.length > 0 ? legalDests.join(',') : '(none)'
    }`,
  );
  unselect(state);
  return false;
...
export function setSelected(state: HeadlessState, key: cg.Key): void {
  state.selected = key;
  //alert(`${key},${isPremovable(state,key)}`);
...
function isPremovable(state: HeadlessState, orig: cg.Key): boolean {
  const piece = state.pieces.get(orig);
  //alert(`${piece},${state.premovable.enabled},${state.movable.color},${state.turnColor}`);
  return (
    !!piece &&
...
export function getKeyAtDomPos(
  ...
  //alert(`${bounds.left},${bounds.width},${bounds.top},${bounds.height},==${pos},${pos2key([file,rank])}`);
