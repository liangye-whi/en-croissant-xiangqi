...
export function renderSvg(state: State, shapesEl: SVGElement, customsEl: SVGElement): void {
  ...
  for (const s of d.shapes.concat(nonPieceAutoShapes).concat(cur ? [cur] : [])) {
    ...
    const sources = dests.get(s.dest) ?? new Set(),
      from = pos2user(orient(key2pos(s.orig), state.orientation)),
      to = pos2user(orient(key2pos(s.dest), state.orientation));
...
function renderShape(
  ...
  const from = pos2user(orient(key2pos(shape.orig), state.orientation)),
    to = shape.dest ? pos2user(orient(key2pos(shape.dest), state.orientation)) : from,
